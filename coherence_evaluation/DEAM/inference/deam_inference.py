...
def load_and_cache_examples(args,
    ...
    cached_features_file = os.path.join(
        args.model_path,
        "cached_{}_{}_{}{}".format(type_data, args.model_type, args.max_length,
                                   additional_filename_postfix))
    ...
    print("len(dataset) is {} in load_and_cache_examples with type_data {}".
          format(len(dataset), type_data))
    return dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--train_data_path',
        default=
        './data/topical_persona/train_amr_manamr_cont_coref_pirel_eng.txt',
        required=False,
        help="path of train conversations")
    parser.add_argument(
        '--valid_data_path',
        default=
        './data/topical_persona/valid_amr_manamr_cont_coref_pirel_eng.txt',
        required=False,
        help="path of valid conversations")
    ...
    parser.add_argument('--max_length',
                        ...
                        type=int,
                        required=False,
                        help="maximum length of input conversations")
    parser.add_argument('--num_labels',
                        default=2,
                        type=int,
                        required=False,
                        help="number of labels for classifying conversations")
    ...
    parser.add_argument('--warmup_decay',
                        default=0.01,
                        required=False,
    ...
    parser.add_argument('--mode',
                        default='valid',
                        required=False,
                        help="mode (train/valid/predict)")
    parser.add_argument('--eval_data_path',
                        required=True,
                        help="path of data to compute coherence evaluation")
    ...
    if args.mode == 'train':
        ...
        model = model_class.from_pretrained(
            args.model_path, num_labels=args.num_labels).to(device)
        print('load model {}'.format(args.model_path))

    print('loading train/valid data ...')
    train_convs, train_orig_convs, train_labels = load_data(
        args.train_data_path)
    valid_convs, valid_orig_convs, valid_labels = load_data(
        args.valid_data_path)
    print('Done!')

    #Create dataset including features for the model
    train_dataset = load_and_cache_examples(args, train_convs, train_labels,
                                            "train")
    valid_dataset = load_and_cache_examples(args, valid_convs, valid_labels,
                                            "valid")

    training_args = TrainingArguments(
        output_dir=args.model_path + '/' + args.model_type,
        num_train_epochs=args.num_epochs,
        per_device_train_batch_size=args.train_batch_size,
        per_device_eval_batch_size=args.valid_batch_size,
        warmup_steps=args.warmup_steps,
        weight_decay=args.warmup_decay,
        learning_rate=args.learning_rate,
        logging_dir=args.model_path + '/logs',
        load_best_model_at_end=True,
        metric_for_best_model=
        'loss',  #use the evaluation loss to save best models
    ...
    if args.mode == 'train':
        ...
        eval_file_name, _ = os.path.splitext(
            os.path.basename(args.eval_data_path))

        tests = {eval_file_name: (args.eval_data_path, )}
        for which_test in tests:
            test_convs, test_orig_convs = load_test_data(
                os.path.join(*tests[which_test]))
            test_dataset = load_and_cache_examples(
                args,
                test_convs,
                labels=None,
                type_data="test",
                additional_filename_postfix="_" + which_test)
            print('Predicting {} which contains {} sentences...'.format(
                which_test, len(test_convs)))
